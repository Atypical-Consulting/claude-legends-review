...
**The gate.** If the framing holds, say nothing *about the framing* and carry on. A check that found
nothing is not news, and reporting it every time turns a safeguard into a tollbooth.

Silence about Step 0 is not silence in general: still tell the user what you're about to review and
that the team is spawning, exactly as you would have without this step. Three agents starting behind
a completely blank response reads as a command that quietly failed.

If a premise is both load-bearing and probably wrong, stop before spawning. Name the premise, say in
one line why it leaves the team's findings unactionable, and offer a concrete choice rather than an
open-ended worry — lettered options let the user answer with a single character instead of a
paragraph. Keep the whole thing readable in one breath; if the objection costs more to read than the
review would have cost to run, you've become the problem you were guarding against.

> Your 13 lines land in `src/Legacy/BillingClient.g.cs`, which carries an `<auto-generated/>` NSwag
> header — the next `nswag run` erases them — and ADR-012 deletes the whole `src/Legacy/` tree next
> sprint. Reviewed as permanent code, the team produces design findings against something with two
> independent expiry dates.
>
> **A** — Review the diff as-is. Right call if this is a knowing stopgap for the next two weeks.
> **B** — Review where this behavior belongs, given the migration. The diff becomes evidence, not the subject.
...
- **Turning Step 0 into a toll booth:** The framing gate earns its place by firing rarely. A lead who opens every review with a paragraph of epistemics has replaced one unexamined premise with a new ritual. Passing quietly is the expected outcome.
- **Mistaking "don't narrate the check" for "say nothing":** Measured on a clean feature diff, the framing wording is strong enough to suppress the lead's normal "here's what I'm reviewing, spawning now" line too. Keep that line — the user is about to spend three agents and deserves to see the scope first.
